...
#     --merge_option merge \
#     --seed 0 \
#     --exp_name ${REGION}_${METHOD}"/train_from_scratch/s'@{seed}'" \
...
python scripts/main.py \
 --config configs/datasets/example_$REGION.yml \
   ...
   --network.pretrained True \
    --network.checkpoint ./results/open-insect-example-ood-oe_resnet50_base_e3_lr0.01_default/s0/best.ckpt \
   --optimizer.num_epochs 1 \
    ...
    --merge_option merge \
    --seed 0 \
    --output_dir $SCRATCH/open_insect_test \
    --exp_name ${REGION}_${METHOD}"/finetune/s'@{seed}'" \
