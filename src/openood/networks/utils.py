...
def get_network(network_config):
    ...
    if network_config.name == "resnet18_32x32":
        ...
        net = WideResNet(
            depth=28, widen_factor=10, dropRate=0.0, num_classes=num_classes
        )
        ...
        module = torch.hub.load(
            "pytorch/vision:v0.9.0", "wide_resnet50_2", pretrained=True
        )
        ...
        model = ReconstructiveSubNetwork(
            in_channels=3, out_channels=3, base_width=int(network_config.image_size / 2)
        )
        ...
        if network_config.image_size == 64:
            netG = Generator(
                1, network_config.nz, network_config.ngf, network_config.nc
            )  # ngpu, nz, ngf, nc
            netD = Discriminator(
                1, network_config.nc, network_config.ndf
            )  # ngpu, nc, ndf
        else:
            netG = Generator32(
                1, network_config.nz, network_config.ngf, network_config.nc
            )  # ngpu, nz, ngf, nc
            netD = Discriminator32(
                1, network_config.nc, network_config.ndf
            )  # ngpu, nc, ndf
        ...
        net = ViT_B_16(num_classes=num_classes, image_size=network_config.image_size)

    elif network_config.name == "swin_t":
        ...
        net = ContrastiveNet(
            backbone,
            network_config.proj_type,
            network_config.hidden_layer_dim,
            network_config.feat_dim,
        )
        ...
        if type(net) is dict:
            ...
            try:
                missing_keys, unexpected_keys = net.load_state_dict(
                    torch.load(network_config.checkpoint, weights_only=True), strict=False
                )
                ...
                loaded_pth = torch.load(network_config.checkpoint, weights_only=True)
