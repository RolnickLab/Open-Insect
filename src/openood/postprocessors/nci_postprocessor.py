...
class NCIPostprocessor(BasePostprocessor):
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        if not self.setup_flag:
            ...
            self.train_mean = torch.from_numpy(np.mean(activation_log_concat, axis=0)).cuda()
    ...
    def postprocess(self, net: nn.Module, data: Any):
        ...
        score = torch.sum(self.w[nn_idx] * (feature - self.train_mean), axis=1) / torch.norm(
            feature - self.train_mean, dim=1
        ) + self.alpha * torch.norm(feature, p=1, dim=1)
        return nn_idx, score
